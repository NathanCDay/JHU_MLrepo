...
##### lift plot comparrison ####
lift_df <- select(test_fin, classe)
lift_df$rf <- predict(rf_fit, test_preds, type = "prob")[,1]
lift_df$gbm <- predict(gbm_fit, test_preds, type = "prob")[,1]
lift_df$xgb <- predict(xgb_fit, test_preds, type = "prob")[,1]

trellis.par.set(caretTheme())
lift_o <- caret::lift(classe ~ rf + gbm + xgb, data = lift_df)
...
library(pROC)
auc(test_fin$xgb, as.factor(test_fin$classe))



# differenct scales need to center and standardize
pp_scale <- preProcess(preds)
train_scale <- predict(pp_scale, preds)




# pca_scale <- prcomp(train_scale)
# 
# summary(pca_scale)
# plot(pca_scale)
# biplot(pca_scale)

pp_scale_pca <- preProcess(train_scale, method = "pca", thresh = .8)
train_scale_pca <- predict(pp_scale_pca, train_scale)

train_fit <- cbind(select(ids, classe), train_scale_pca)
train_fit %<>% filter(complete.cases(.))

knn_fit <- train(classe ~ ., method = "knn", data = train_fit)
...
testy <- all[-index, ]
testy %<>% select(-c(1,3:6))
ids <- select(testy, c(1:2, 155))
preds <- select(testy, -c(1:2, 155))
preds %<>% dplyr::select(good_cols)
preds %<>%  select(-grep(T, nsv$nzv))
pp_scale <- preProcess(preds)
test_scale <- predict(pp_scale, preds)
test_scale_pca <- predict(pp_scale_pca, test_scale)

test_ready <- cbind(select(ids, classe), test_scale_pca)
test_ready %<>% filter(complete.cases(.))

test_ready$pred <- predict(knn_fit, test_ready)

confusionMatrix(test_ready$pred, test_ready$classe)
